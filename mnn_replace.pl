...
compare_outputs(TestSet, NNOut, MNNOut, Accuracy, Agreement, Errors) :-
    zip_test_outputs(TestSet, NNOut, MNNOut, Zipped),
    length(Zipped, Total),
    include(matches_expected_mnn, Zipped, MnnMatches),
    length(MnnMatches, MnnCorrect),
    include(matches_nn_mnn, Zipped, Agreements),
    length(Agreements, AgreeCount),
    findall(E, (member(Item, Zipped), mismatch_item(Item, E)), Errors),
    ( Total =:= 0
    -> Accuracy = 0.0, Agreement = 0.0
    ; Agreement is AgreeCount / Total,
      ( has_labeled_examples(Zipped)
      -> Accuracy is MnnCorrect / Total
      ; Accuracy = Agreement
      )
    ).

zip_test_outputs([], [], [], []).
zip_test_outputs([Input|I], [N|Ns], [M|Ms], [item(Input, N, M)|T]) :-
    zip_test_outputs(I, Ns, Ms, T).

matches_expected_mnn(item(_Input-Expected, _N, M)) :-
    nonvar(Expected),
    \+ mode_mismatch(Expected, M).
matches_expected_mnn(item(_Input, _N, _M)) :-
    fail.

matches_nn_mnn(item(_Input, N, M)) :-
    \+ mode_mismatch(N, M).

has_labeled_examples(Items) :-
    member(item(_Input-Expected, _, _), Items),
    nonvar(Expected),
    !.

mismatch_item(item(Input-Expected, N, M), Error) :-
    nonvar(Expected),
    ( mode_mismatch(Expected, M)
    -> Error = error(Input, expected(Expected), nn(N), mnn(M))
    ; fail
    ).
mismatch_item(item(Input, N, M), Error) :-
    Input \= (_-_),
    mode_mismatch(N, M),
    Error = error(Input, nn(N), mnn(M)).
...
mnn_benchmark(NN, MNN, benchmark(or_problem, OriginalAccuracy, ReplacementAccuracy, Agreement,
    ...
    nn_accuracy(NN, TestSet, OriginalAccuracy),
    compare_nn_mnn(NN, MNN, TestSet,
                   report(ReplacementAccuracy, Agreement, _Errors,
...
make_testset(Inputs, Inputs).

verification_kind(verified, formal).
verification_kind(verified_bounded(_), bounded).
verification_kind(empirically_equivalent(_), empirical).
verification_kind(failed(_), failed).
verification_kind(unknown, unknown).

next_experiment_dir(Dir) :-
    between(1, 9999, N),
    format(atom(Dir), 'results/experiment-~|~`0t~d~3+', [N]),
    \+ exists_directory(Dir),
    !.

nn_accuracy(NN, TestSet, Accuracy) :-
    maplist(nn_matches_expected(NN), TestSet, Matches),
    include(=(true), Matches, Trues),
    length(Matches, Total),
    length(Trues, Correct),
    (Total =:= 0 -> Accuracy = 0.0 ; Accuracy is Correct / Total).

nn_matches_expected(NN, Input-Expected, Match) :-
    nn_input(NN, Input, Out),
    ( mode_mismatch(Expected, Out) -> Match = false ; Match = true ).
...
parse_cli(Argv, Model, Options) :-
    ( append(_, ['--model', ModelRaw|_], Argv) -> normalize_arg_atom(ModelRaw, Model) ; Model = model1 ),
...
cli_option(Argv, component(Component)) :-
    append(_, ['--component', ComponentRaw|_], Argv),
    normalize_arg_atom(ComponentRaw, Component).

normalize_arg_atom(Raw, Atom) :-
    ( atom(Raw) -> Atom = Raw
    ; string(Raw) -> atom_string(Atom, Raw)
    ; term_to_atom(Raw, Atom)
    ).
